...
class DonationBackendContractTests(unittest.TestCase):
    ...
    def test_retry_ties_and_recurring_failure_attribution_are_locked(self) -> None:
        ...
        status_model = (validate_donations.ROOT / "functions/src/donation-status.ts").read_text(encoding="utf-8")
        behavior = (validate_donations.ROOT / "functions/scripts/test-donation-status.mjs").read_text(encoding="utf-8")
        self.assertIn("resolveDonationStatusTransition", donations)
        self.assertIn("staleRecurringCancellationOverride", donations)
        self.assertIn("recurringCancellationEventCreated", donations)
        self.assertIn("recurringCancellationEventId", donations)
        self.assertIn("successfulRetryAtSameTime", status_model)
        self.assertIn("staleFailureAfterSameTimeSuccess", status_model)
        self.assertIn("TERMINAL_DISPUTE_STATUSES", status_model)
        self.assertIn("if (TERMINAL_DISPUTE_STATUSES.has(status)) return 8", status_model)
        self.assertIn("RECURRING_CANCELLATION_OVERRIDABLE_STATUSES", status_model)
        self.assertIn("requestedStatus === 'recurring_cancelled'", status_model)
        self.assertIn("(!staleByTime || staleRecurringCancellationOverride)", status_model)
        self.assertIn("input.priorEventId || input.eventId", status_model)
        self.assertIn("decreasesRefundTotal", status_model)
        ...
        self.assertIn("cancellation is stored first", behavior)
        self.assertIn("a newer failure arrives first", behavior)
        self.assertIn("staleCancellationAfterRefund", behavior)
        self.assertIn("staleCancellationAfterDispute", behavior)
        self.assertIn("ordinaryStaleEvent", behavior)
        self.assertNotIn(r"\\n", donations)
        self.assertNotIn(r"\\n", status_model)
