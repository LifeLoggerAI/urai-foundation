(() => {
  const state = { stage: "profile", opportunity: "community-access", builtForOpportunity: null, draftStale: false, version: 1, approvedVersions: new Set(), signed: false, signedAt: null, approvedPayload: null, answers: [] };
  ...
  function updateAnswer(index, value, renderReviewAfter = true) {
    ...
    const changedApprovedDraft = nextValue !== answer.value && state.approvedVersions.has(state.version);
  ...
  function signApplication() {
    ...
    const signer = byId("signatureName").value.trim(); state.signed = true; state.signedAt = new Date().toISOString(); state.approvedVersions.add(state.version); state.approvedPayload = createExportPayload(signer, state.signedAt); const template = opportunityTemplates[state.opportunity]; const receipt = byId("signoffReceipt"); receipt.hidden = false;
